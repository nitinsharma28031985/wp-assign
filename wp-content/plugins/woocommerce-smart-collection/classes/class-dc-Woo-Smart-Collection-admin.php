...
class DC_Woo_Smart_Collection_Admin {
  ...
  function set_woo_smart_collection_options($product) {
    ...
    echo '<table>';
    if(isset($smart_cat_settings['is_append'])){
        $dfappend = $smart_cat_settings['is_append'];
    } else {
        $dfappend = '';
    }
    if(isset($smart_cat_settings['is_enable'])){
        $dffEnable = $smart_cat_settings['is_enable'];
    }  else {
        $dffEnable = '';
    }
    if(isset($smart_cat_settings['is_excerpt'])){
        $dffexcerpt = $smart_cat_settings['is_excerpt'];
    } else {
        $dffexcerpt = '';
    }
    if(isset($smart_cat_settings['is_content'])){
       $dffcontent = $smart_cat_settings['is_content'];
    }else{
        $dffcontent = '';
    }
    if(isset($smart_cat_settings['is_tag'])){
        $dfftag = $smart_cat_settings['is_tag'];
    }else{
        $dfftag = '';
    }
    if(isset($smart_cat_settings['is_title'])){
        $dfftitle = $smart_cat_settings['is_title'];
    }else{
        $dfftitle = '';
    }
    $settings_options = array(
                             "placeholder" => array('type' => 'hidden', 'name' => 'smart_cat_settings[placeholder]', 'value' => 'placeholder'),
                             "is_enable" => array('label' => __('Enable Smart Category', $DC_Woo_Smart_Collection->text_domain), 'type' => 'checkbox', 'name' => 'smart_cat_settings[is_enable]', 'value' => 'Enable', 'dfvalue' => $dffEnable),
                             "is_append" => array('label' => __('Append with existing smart categories', $DC_Woo_Smart_Collection->text_domain), 'type' => 'checkbox', 'name' => 'smart_cat_settings[is_append]', 'value' => 'Append', 'dfvalue' => $dfappend, 'hints' => __('If unchecked will replace existing smart categories', $DC_Woo_Smart_Collection->text_domain)),
                             "is_title" => array('label' => __('Generate Smart Category from Post Title', $DC_Woo_Smart_Collection->text_domain), 'type' => 'checkbox', 'name' => 'smart_cat_settings[is_title]', 'value' => 'Title', 'dfvalue' => $dfftitle),
                             "is_excerpt" => array('label' => __('Generate Smart Category from Post Excerpt', $DC_Woo_Smart_Collection->text_domain), 'type' => 'checkbox', 'name' => 'smart_cat_settings[is_excerpt]', 'value' => 'Excerpt', 'dfvalue' => $dffexcerpt),
                             "is_content" => array('label' => __('Generate Smart Category from Post Content', $DC_Woo_Smart_Collection->text_domain), 'type' => 'checkbox', 'name' => 'smart_cat_settings[is_content]', 'value' => 'Content', 'dfvalue' =>$dffcontent),
                             "is_tag" => array('label' => __('Generate Smart Category from Post Tags', $DC_Woo_Smart_Collection->text_domain), 'type' => 'checkbox', 'name' => 'smart_cat_settings[is_tag]', 'value' => 'Tag', 'dfvalue' => $dfftag)
                             );
